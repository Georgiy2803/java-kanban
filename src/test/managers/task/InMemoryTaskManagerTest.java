...
public class InMemoryTaskManagerTest {
    ...
    void getTaskById_returnSameTaskcWithSAmeFields(){
        ...
        assertEquals(task.getName(), manager.getTaskById(task.getId()).get().getName(), "Наименования Task не совпадают");
        assertEquals(task.getDescription(), manager.getTaskById(task.getId()).get().getDescription(), "Описание Task не совпадают");
        assertEquals(task.getId(), manager.getTaskById(task.getId()).get().getId(), "id Task не совпадают");
        assertEquals(task.getStatus(),manager.getTaskById(task.getId()).get().getStatus(),"Статус Task не совпадают");
    }
    ...
    void getEpicById_returnSameEpicObject() { // get Epic By Id_end the same Epic object
        ...
        assertEquals(epic, manager.getEpicById(epic.getId()).get(), "Эпики не совпадают");
    }
    ...
    void getEpicById_returnSameEpicWithSAmeFields(){
        ...
        assertEquals(epic.getId(), manager.getEpicById(epic.getId()).get().getId(), "id Эпик не совпадают");
        assertEquals(epic.getStatus(),manager.getEpicById(epic.getId()).get().getStatus(),"Статус Эпик не совпадают");
        assertEquals(epic.getListSubtaskIds(), manager.getEpicById(epic.getId()).get().getListSubtaskIds(),"Списки подзадач Эпика не совпадают");
    }
    ...
    void getSubtaskById_returnSameSubtaskWithSAmeFields(){
        ...
        Subtask subtask = manager.createSubtask(new Subtask("Название подзазачи", "Описание подзазачи", epic.getId() ));
        // проверка, что переданные поля при создании метода, совпадают с теми, что есть в менеджере
        assertEquals(subtask.getName(), manager.getSubtaskById(subtask.getId()).get().getName(), "Наименования подзазачи не совпадают");
        assertEquals(subtask.getDescription(), manager.getSubtaskById(subtask.getId()).get().getDescription(), "Описание подзазачи не совпадают");
        assertEquals(subtask.getId(), manager.getSubtaskById(subtask.getId()).get().getId(), "id подзазачи не совпадают");
        assertEquals(subtask.getStatus(),manager.getSubtaskById(subtask.getId()).get().getStatus(),"Статус подзазачи не совпадают");
        assertEquals(subtask.getEpicId(),manager.getSubtaskById(subtask.getId()).get().getEpicId(),"id связанного Эпик не совпадают");
    }
    ...
    void getHistoryTask_returnOldUpdateTask() { // HistoryManager сохраняет предыдущую версию задачи и её данных
        ...
        assertEquals(task1, manager.getHistory().get(0), "Task не совпадают");
    }
    ...
    void getHistoryTask_returnSameTaskWithSAmeFields() {  // Полученная задача из истории соответствуют задаче в менеджере
        ...
        manager.getTaskById(task1.getId()); // просматриваем задачу
        // проверяем поля
        assertEquals(1, manager.getHistory().get(0).getId());
        assertEquals("Имя 1", manager.getHistory().get(0).getName());
        assertEquals("Описание 1", manager.getHistory().get(0).getDescription());
        assertEquals(Status.NEW, manager.getHistory().get(0).getStatus());
    }
    ...
    void getHistorySubtask_returnSameSubtaskWithSAmeFields() {  // Полученный Subtask из истории соответствуют Subtask в менеджере
        ...
        // проверяем поля
        assertEquals(2, manager.getHistory().get(0).getId());
        assertEquals("Подзадача 1", manager.getHistory().get(0).getName());
        assertEquals("Описание подзадачи 1", manager.getHistory().get(0).getDescription());
        assertEquals(Status.NEW, manager.getHistory().get(0).getStatus());
    }
}
